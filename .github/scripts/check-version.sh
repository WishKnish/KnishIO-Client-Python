#!/usr/bin/env bash
#
# Version consistency gate for the KnishIO Python SDK.
#
...
collect "knishioclient/__init__.py __version__" \
  "$(sed -n "s/^__version__ *= *['\"]\([^'\"]*\)['\"].*/\1/p" knishioclient/__init__.py | head -1)"
# -----------------------------------------------------------------------------------------
